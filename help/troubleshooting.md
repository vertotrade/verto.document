...
> The transaction cannot succeed due to error: VertoRouter: INSUFFICIENT\_OUTPUT\_AMOUNT. This is probably an issue with one of the tokens you are swapping.
>
...
3. Increase your slippage tolerance:
   ...
   2. Increase your slippage tolerance a little and try again. ![Increase Slippage](/public/assets/trouble-increase-slippage.png)
...
> Fail with error 'VertoRouter: INSUFFICIENT\_A\_AMOUNT'\
> or\
> Fail with error 'VertoRouter: INSUFFICIENT\_B\_AMOUNT'
...
![Increase Slippage](/public/assets/trouble-increase-slippage.png)
{% endtab %}
...
There currently isn't a simple way to solve this issue from the VertoTrade website: you'll need to interact with the contract directly. You can add liquidity directly via the Router contract, while setting amountAMin to a small amount, then withdrawing all liquidity.
...
![](/public/assets/trouble-query-balanceof.png)
...
![](/public/assets/trouble-add-remove-liquidity.png)
...
![Verto K Solution](/public/assets/verto-k-solution.png)

This usually happen when you are trying to swap a token with its own fee.

### Verto: TRANSFER\_FAILED

> The transaction cannot succeed due to error: execution reverted: Verto: TRANSFER\_FAILED.
...
{% tab title="Reason" %}
**This issue (while swapping) is caused by tokens which have hard-coded the V1 VertoTrade router into their contract.**
...
You don't have enough REBUS in your wallet to unstake from the VERTO-REBUS pool.

**Get at least as much REBUS as the amount of VERTO that you’re trying to unstake.**

1. Buy REBUS on the exchange. If you want to unstake 100 VERTO, you need at least 100 REBUS.
...
2. Click **“Connect to Web3”** and connect your wallet. ![Connect to Web3](https://lh6.googleusercontent.com/-\_sNkO1gcOOJXkduDEUzbExKE2mNxBOR0f86Lpp3BBuPbIcmAHsfuvpF-hKqRn4oID5QzdGkk\_1dTHkPuCmE50vpNNZxEqoM5nPmE\_12k3-8Q8YYoRYqJ\_VGjxJ03YPRuVQ1O5ME)
3. In section **“4. emergencyWithdraw”**, enter "0" and click “Write”.

This will unstake your staked tokens and lose any uncollected VERTO yield.

{% hint style="warning" %}
**This will lose any yield that you haven’t harvested yet.**
{% endhint %}

To stop this happening again, **don’t sell your REBUS.** You still need it to unstake from the “Stake VERTO Earn VERTO” pool.

This error has happened because you have sold or transferred REBUS tokens. REBUS is minted in a 1:1 ratio to VERTO when you stake in the VERTO-VERTO Minting Pools. REBUS must be burned at a 1:1 ratio to VERTO when calling leaveStaking (unstaking your VERTO from the pool), so if you don't have enough, you can't unstake from the pool.
...
![](/public/assets/trouble-out-of-gas.png)
...
You're probably trying to unstake from a Minting Pools with low rewards in it. Solution below.
...
1. Find the contract address of the Minting Pools you're trying to unstake from. You can find it in your wallet's transaction log.
...
{% tab title="Reason" %}
This error tends to appear when you're trying to unstake from an old Minting Pools, but there aren't enough rewards in the pool left for you to harvest when withdrawing. This causes the transaction to fail.
{% endtab %}
{% endtabs %}
-->
...
{% tabs %}
{% tab title="Solution" %}
Install the official browser extension to connect, or read our guide on [how to connect a wallet to VertoTrade](https://docs.vertotrade.com/get-started/connection-guide).
{% endtab %}
{% endtabs %}
...
<!--
// TODO add when marketplace is up
## **Issues with Profile**

### Oops! We couldn't find any Verto Collectibles in your wallet
...
1. Go to “Collectible” page, then come back to profile page.\
   If you can’t find the link, go to [https://vertotrade.com/collectibles](https://vertotrade.com/collectibles) directly.
...
\
It may make a conflict between wallets. This is out of VertoTrade's control and we can do nothing.
